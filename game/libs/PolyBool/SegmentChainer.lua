...
local function SegmentChainer(segments, eps, buildLog)
    ...
    for _, seg in next, segments do
        ...
        if (buildLog) then buildLog.chainStart(seg) end
        ...
        for i = 1, #chains do
            ...
            if (eps.pointsSame(head, pt1)) then
                if (setMatch(i, true, true)) then break end
            elseif (eps.pointsSame(head, pt2)) then
                if (setMatch(i, true, false)) then break end
            elseif (eps.pointsSame(tail, pt1)) then
                if (setMatch(i, false, true)) then break end
            elseif (eps.pointsSame(tail, pt2)) then
                if (setMatch(i, false, false)) then break end
            end
        end
        ...
        if (next_match == first_match) then
            ...
            if (buildLog) then buildLog.chainNew(pt1, pt2) end
        elseif (next_match == second_match) then
            -- we matched a single chain

            if (buildLog) then buildLog.chainMatch(first_match.index) end
            ...
            local function reverseChain(index)
                if (buildLog) then buildLog.chainReverse(index) end
            ...
            if (buildLog) then buildLog.chainConnect(F, S) end
